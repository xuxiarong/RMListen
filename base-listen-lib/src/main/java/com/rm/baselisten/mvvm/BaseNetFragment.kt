...
import com.rm.baselisten.databinding.BaseFragmentNetBinding
 ...
 * version: 1.0
 */
abstract class BaseNetFragment<V : ViewDataBinding, VM : BaseNetViewModel> : Fragment() {


    /**
     * 初始化base的dataBind对象，并且注册lifecycle
     */
    private lateinit var baseBinding: BaseFragmentNetBinding
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        baseBinding = DataBindingUtil.inflate<BaseFragmentNetBinding>(inflater, R.layout.base_fragment_net, container, false).apply {
            lifecycleOwner = this@BaseNetFragment
        }
    ...
    private fun startBaseObserve() {
        ...
        mViewModel.baseStatusModel.observe(this@BaseNetFragment, Observer {
            setStatus(it)
        })

        mViewModel.baseTitleModel.observe(this@BaseNetFragment, Observer {
            setTitle()
        })
    }
