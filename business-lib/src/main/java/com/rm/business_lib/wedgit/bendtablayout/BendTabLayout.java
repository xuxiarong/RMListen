...
public class BendTabLayout extends HorizontalScrollView {
    ...
    private static final Pools.Pool<BendTab> sTabPool = new Pools.SynchronizedPool<>(16);
    ...
    /**
     * Gravity used to fill the {@link BendTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link BendTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    public interface OnTabSelectedListener {
         ...
         */
        public void onTabSelected(BendTab tab);
         ...
         */
        public void onTabUnselected(BendTab tab);
         ...
         */
        public void onTabReselected(BendTab tab);
    }

    private final ArrayList<BendTab> mTabs = new ArrayList<>();
    private BendTab mSelectedTab;
    ...
    public BendTabLayout(Context context) {
        this(context, null);
    }

    public BendTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    @SuppressLint("PrivateResource")
    public BendTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabStrip.setIndicatorAnimationSupport(a.getBoolean(R.styleable.BendTabLayout_bendTabIndicatorAnimation, false));


        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.BendTabLayout_bendTabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.BendTabLayout_bendTabPaddingStart, mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.BendTabLayout_bendTabPaddingTop, mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.BendTabLayout_bendTabPaddingEnd, mTabPaddingEnd);
        ...
        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.BendTabLayout_bendTabMinWidth, INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.BendTabLayout_bendTabMaxWidth, INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.BendTabLayout_bendTabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.BendTabLayout_bendTabContentStart, 0);
        mMode = a.getInt(R.styleable.BendTabLayout_bendTabMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.BendTabLayout_bendTabGravity, GRAVITY_FILL);
        mIndicatorMarginTop = a.getDimensionPixelSize(R.styleable.BendTabLayout_indicatorMarginTop, mIndicatorMarginTop);
        a.recycle();

        // add attr for these
        final Resources res = getResources();
        mTabTextMultiLineSize = res.getDimensionPixelSize(R.dimen.bend_tab_text_size_2line);
        mScrollableTabMinWidth = res.getDimensionPixelSize(R.dimen.bend_tab_scrollable_min_width);
     ...
     */
    public void addTab(@NonNull BendTab tab) {
     ...
     */
    public void addTab(@NonNull BendTab tab, int position) {
     ...
     */
    public void addTab(@NonNull BendTab tab, boolean setSelected) {
     ...
     * @param setSelected True if the added tab should become the selected tab.
     */
    public void addTab(@NonNull BendTab tab, int position, boolean setSelected) {
        if (tab.mParent != this) {
    ...
    private void addTabFromItemView(@NonNull BendTabItem item) {
        final BendTab tab = newTab();
        if (item.mText != null) {
    ...
    /**
     * Add a {@link BendTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     * <p>
    ...
    /**
     * Remove the given {@link BendTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link BendTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Create and return a new {@link BendTab}. You need to manually add this using
     * {@link #addTab(BendTab)} or a related method.
     *
     * @return A new Tab
     * @see #addTab(BendTab)
     */
    @NonNull
    public BendTab newTab() {
        BendTab tab = sTabPool.acquire();
        if (tab == null) tab = new BendTab();
        tab.mParent = this;
     ...
     */
    @Nullable
    public BendTab getTabAt(int index) {
     ...
     */
    public void removeTab(BendTab tab) {
        if (tab.mParent != this) {
    ...
    public void removeTabAt(int position) {
        ...
        final BendTab removedTab = mTabs.remove(position);
        if (removedTab != null) {
            removedTab.reset();
    ...
    public void removeAllTabs() {
        ...
        for (final Iterator<BendTab> i = mTabs.iterator(); i.hasNext(); ) {
            final BendTab tab = i.next();
            i.remove();
            tab.reset();
    ...
    /**
     * Returns the current mode used by this {@link BendTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link BendTabLayout} with a {@link ViewPager}.
     * <p>
    ...
    private void configureTab(BendTab tab, int position) {
    ...
    private void addTabView(BendTab tab) {
    ...
    private void addViewInternal(final View child) {
        if (child instanceof BendTabItem) {
            addTabFromItemView((BendTabItem) child);
        } else {
    ...
    private void animateToTab(int newPosition) {
        if (newPosition == BendTab.INVALID_POSITION) {
            return;
        }
    ...
    void selectTab(BendTab tab) {
        selectTab(tab, true);
    }

    void selectTab(final BendTab tab, boolean updateIndicator) {
        final BendTab currentTab = mSelectedTab;
        ...
        if (currentTab == tab) {
            ...
            final int newPosition = tab != null ? tab.getPosition() : BendTab.INVALID_POSITION;
            if (updateIndicator) {
                if ((currentTab == null || currentTab.getPosition() == BendTab.INVALID_POSITION) && newPosition != BendTab.INVALID_POSITION) {
    ...
    private void dispatchTabSelected(@NonNull final BendTab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
            mSelectedListeners.get(i).onTabSelected(tab);
        }
    }

    private void dispatchTabUnselected(@NonNull final BendTab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
            mSelectedListeners.get(i).onTabUnselected(tab);
        }
    }

    private void dispatchTabReselected(@NonNull final BendTab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
     ...
     */
    public static final class BendTab {
         ...
         */
        @NonNull
        public BendTab setTag(@Nullable Object tag) {
            mTag = tag;
            return this;
        }
         ...
         */
        @NonNull
        public BendTab setCustomView(@Nullable View view) {
            mCustomView = view;
            updateView();
            return this;
        }
         ...
         */
        @NonNull
        public BendTab setCustomView(@LayoutRes int resId) {
         ...
         */
        @NonNull
        public BendTab setIcon(@Nullable Drawable icon) {
            mIcon = icon;
            updateView();
            return this;
        }
         ...
         */
        @NonNull
        public BendTab setText(@Nullable CharSequence text) {
            mText = text;
            updateView();
            return this;
        }
         ...
         */
        @NonNull
        public BendTab setContentDescription(@Nullable CharSequence contentDesc) {
    ...
    public class TabView extends LinearLayout {
        ...
        void setTab(@Nullable final BendTab tab) {
            if (tab != mTab) {
                mTab = tab;
                update();
            }
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    ...
                    if (mIsSupportIndicatorAnimation) {
                        ...
                        // Draw the selection partway between the tabs
                        left = (int) (mSelectionOffset * (nextTitle.getLeft() + (nextTitle.getWidth() - mSelectedIndicatorWidth) / 2) + (1.0f - mSelectionOffset) * left);
                        right = left + mSelectedIndicatorWidth;
                    }
...
//                // 填充二阶贝塞尔曲线的另外两个控制点 B(bx,by) 和 C(cx,cy)，切记顺序不能变
                path.quadTo(mIndicatorRight - ((mIndicatorRight - mIndicatorLeft) / 2), getBottom()-5, mIndicatorRight, getHeight() - mSelectedIndicatorHeight - mSelectedIndicatorPaint.getStrokeWidth());
                // 将 贝塞尔曲线 绘制至画布
     ...
     * necessary calls back to the provided {@link BendTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<BendTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(BendTabLayout tabLayout) {
        ...
        public void onPageScrollStateChanged(final int state) {
            ...
            mScrollState = state;
            BendTabLayout tabLayout = mTabLayoutRef.get();
        ...
        public void onPageScrolled(final int position, final float positionOffset, final int positionOffsetPixels) {
            final BendTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
    ...
    /**
     * A {@link BendTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements BendTabLayout.OnTabSelectedListener {
