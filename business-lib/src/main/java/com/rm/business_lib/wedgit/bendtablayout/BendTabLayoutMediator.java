 ...
 * TabLayoutMediator.
 */
public final class BendTabLayoutMediator {
  @NonNull private final BendTabLayout tabLayout;
  ...
  @Nullable private BendTabLayout.OnTabSelectedListener onTabSelectedListener;
  ...
  public interface TabConfigurationStrategy {
     ...
     * BendTabLayout.BendTab#setText(CharSequence)}, but any form of styling can be applied.
     *
     * @param tab The Tab which should be configured to represent the title of the item at the given
     *     position in the data set.
     * @param position The position of the item within the adapter's data set.
     */
    void onConfigureTab(@NonNull BendTabLayout.BendTab tab, int position);
  }

  public BendTabLayoutMediator(
      @NonNull BendTabLayout tabLayout,
  ...
  void populateTabsFromPagerAdapter() {
    ...
    if (adapter != null) {
      ...
      for (int i = 0; i < adapterCount; i++) {
        BendTabLayout.BendTab tab = tabLayout.newTab();
  ...
  private static class TabLayoutOnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
    @NonNull private final WeakReference<BendTabLayout> tabLayoutRef;
    private int previousScrollState;
    private int scrollState;

    TabLayoutOnPageChangeCallback(BendTabLayout tabLayout) {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
      BendTabLayout tabLayout = tabLayoutRef.get();
      if (tabLayout != null) {
   ...
   */
  private static class ViewPagerOnTabSelectedListener implements BendTabLayout.OnTabSelectedListener {
    ...
    @Override
    public void onTabSelected(@NonNull BendTabLayout.BendTab tab) {
      viewPager.setCurrentItem(tab.getPosition(), smoothScroll);
    }

    @Override
    public void onTabUnselected(BendTabLayout.BendTab tab) {
      // No-op
    }

    @Override
    public void onTabReselected(BendTabLayout.BendTab tab) {
      // No-op
    }
  }
