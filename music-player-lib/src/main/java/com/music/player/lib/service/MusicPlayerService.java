...
public class MusicPlayerService extends Service implements MusicPlayerPresenter,MediaPlayer.OnPreparedListener,
    ...
    public void onCreate() {
        ...
        if(wifiManager!=null){
            mWifiLock = wifiManager.createWifiLock(WifiManager.WIFI_MODE_FULL, "MUSIC_LOCK");
        }
    ...
    private void initPlayerConfig(){
        ...
        switch (value) {
            case MusicConstants.SP_VALUE_MUSIC_MODEL_SINGLE:
                mLoop = true;
                model = MusicConstants.MUSIC_MODEL_SINGLE;
                break;
            case MusicConstants.SP_VALUE_MUSIC_MODEL_LOOP:
                model = MusicConstants.MUSIC_MODEL_LOOP;
                mLoop = false;
                break;
            case MusicConstants.SP_VALUE_MUSIC_MODEL_ORDER:
                model = MusicConstants.MUSIC_MODEL_ORDER;
                mLoop = false;
                break;
            case MusicConstants.SP_VALUE_MUSIC_MODEL_RANDOM:
        ...
        if (null != mAudios && mAudios.size() > 0) {
            switch (getPlayerState()) {
                ...
                case MusicConstants.MUSIC_PLAYER_PREPARE:
                    Log.e(TAG, "playOrPause: MusicConstants.MUSIC_PLAYER_PREPARE" );
                    pause();
                    break;
                case MusicConstants.MUSIC_PLAYER_BUFFER:
                    Log.e(TAG, "playOrPause: MusicConstants.MUSIC_PLAYER_BUFFER" );
                    pause();
                    break;
                case MusicConstants.MUSIC_PLAYER_PLAYING:
                    Log.e(TAG, "playOrPause: MusicConstants.MUSIC_PLAYER_PLAYING" );
                    pause();
                    break;
                ...
                case MusicConstants.MUSIC_PLAYER_ERROR:
                    Log.e(TAG, "playOrPause: MusicConstants.MUSIC_PLAYER_ERROR" );
                    startPlayMusic(mCurrentPlayIndex);
                    break;
            }
        }
    ...
    public int playLastIndex() {
        ...
        if (null != mAudios && mAudios.size() > 0) {
            switch (getPlayerModel()) {
                ...
                //随机
                case MusicConstants.MUSIC_MODEL_RANDOM:
                    tempIndex= MusicUtils.getInstance().getRandomNum(0, mAudios.size() - 1);
                    break;
            }
        }
    ...
    private synchronized void startPlay(BaseAudioInfo musicInfo) {
        ...
        if(null!=musicInfo&&!TextUtils.isEmpty(musicInfo.getAudioPath())){
            ...
            MusicPlayerManager.getInstance().observerUpdata(new MusicStatus(MusicStatus.PLAYER_STATUS_ERROR,
                    musicInfo == null? 0L : musicInfo.getAudioId()));
        }
    }
...
//        }
    ...
    public boolean isCheckNetwork(){
        ...
        return type == ConnectivityManager.TYPE_MOBILE || type == ConnectivityManager.TYPE_WIFI;
    }
    ...
    private class HeadsetBroadcastReceiver extends BroadcastReceiver{
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(AudioManager.ACTION_AUDIO_BECOMING_NOISY.equals(action)){
                ...
                //前台进程-通知栏根点击事件
            }else if(MusicConstants.MUSIC_INTENT_ACTION_ROOT_VIEW.equals(action)){
                ...
                //前台进程-上一首
            }else if(MusicConstants.MUSIC_INTENT_ACTION_CLICK_LAST.equals(action)){
                playLastMusic();
                //前台进程-下一首
            }else if(MusicConstants.MUSIC_INTENT_ACTION_CLICK_NEXT.equals(action)){
                playNextMusic();
                //前台进程-暂停、开始
            }else if(MusicConstants.MUSIC_INTENT_ACTION_CLICK_PAUSE.equals(action)){
                playOrPause();
                //前台进程-关闭前台进程
            }else if(MusicConstants.MUSIC_INTENT_ACTION_CLICK_CLOSE.equals(action)){
                cleanNotification();
                //收藏
            }else if(MusicConstants.MUSIC_INTENT_ACTION_CLICK_COLLECT.equals(action)){
